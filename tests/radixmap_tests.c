...
static int check_order(RadixMap * map)
{
    ...
    // only signal errors if any (should net be)
...
static char *test_operations()
{
    ...
    mu_assert(check_order(map), 
              "RadixMap didn't stay sorted ater search.");
    
    
    while (map->end > 0) {
        RMElement *el = RadixMap_find(map,
            map->contents[map->end / 2]. data.key);
        mu_assert(el != NULL, "Shoud get a result");
        
        size_t old_end = map->end;
        
        mu_assert(RadixMap_delete(map, el) == 0, "Didn't delelte it.");
